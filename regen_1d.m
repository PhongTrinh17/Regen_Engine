...
Pamb = 9.94; % psia Geo.At 10,500 ft altitude
...
temps_cea = c.get_Temperatures(pyargs('Pc', Pc_us, 'MR', o_f, 'eps', eps)); % Mat.r
...
Geo.Ae = Geo.At * eps; % m^2
Geo.Re = sqrt(Geo.Ae/pi); % m
Geo.percent_len = 0.8; % input (0.8 is optimal fractional length for most cases)
Geo.len_div = Geo.percent_len * ((Geo.Re - Geo.Rt)/tan(deg2rad(15))); % m diverging length from radii
...
Geo.matrix_B = [Geo.Rn; Geo.Re; tan(Geo.theta_n)];
...
Geo.D_t = 2 * Geo.Rt;
Geo.h_channel = Geo.min_tol; % channel height (radial)
...
%% HT Areas
Geo.A_gas = pi .* Geo.D_gas .* Geo.dx; % Gas-wall convection SA
Geo.A_w = pi .* ((Geo.D_gas + Geo.D_channel_base)./2) .* Geo.dx; % wall-wall conduction SA (average diameter)
Geo.A_co = Geo.Per_heated .* Geo.num_channel .* Geo.dx; % coolant side surface area (heated)
Geo.A_wc = Geo.w_channel .* Geo.dx; % cool wall area per increment
...
if isfile(table_filename)
    ...
    T_sat_clean = T_sat_grid(Cool.psat_index);
...
while abs(Loop.P_error) > Loop.tol_P % Pressure guess loop
    ...
    for d = length(Geo.pos_i):-1:1 % Axial marching loop
        ...
        % Coolant properties
        Loop.rho_c = get_rho(Loop.P_loc, Loop.T_bulk);
        Loop.cp_c = get_cp(Loop.P_loc, Loop.T_bulk);
        Loop.mu_c = get_mu(Loop.P_loc, Loop.T_bulk);
        Loop.k_c = get_k(Loop.P_loc, Loop.T_bulk);
        Loop.T_sat = get_T_sat(Loop.P_loc);
        Loop.rho_c_l = get_rho_l(Loop.P_loc);
        Loop.rho_c_v = get_rho_v(Loop.P_loc);
        Loop.surften = get_surften(Loop.P_loc);
        Loop.h_fg = get_h_fg(Loop.P_loc);
...
hold on; grid on;
plot(Geo.pos_i, Arrays.h_g_array, 'r', 'LineWidth', 2);
...
function Temp = temp_iteration_sc(Geo, Gas, Loop, Mat, d, Param) % HW Temp Iteration, subcooled
    ...
    while abs(T_hw_error) > tol_T_hw
        ...
        %fin_m = sqrt((2*Loop.h_c*(Geo.dx + Geo.w_rib))/(Temp.k_w_loc * Geo.dx * Geo.w_rib));
